...
def list_records(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    return (
        db.query(Record).filter(Record.user_id == user.id).order_by(Record.updated_at.desc()).all()
    )
...
def get_one(record_id: str, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    a = db.query(Record).filter(Record.id == record_id, Record.user_id == user.id).first()
    if not a:
...
def update(
    ...
    if data.staff_count is not None:
        if data.staff_count < 1 or data.staff_count > 10:
            raise HTTPException(400, {"code": "STAFF_RANGE", "message": "staff_count must be 1-10"})
    ...
    if a.period_start > a.period_end:
        raise HTTPException(400, {"code": "INVALID_SCALE", "message": "Invalid year range"})
    if scale_changed and (a.period_end - a.period_start) > 20:
        raise HTTPException(
            400, {"code": "SCALE_TOO_WIDE", "message": "Range too wide (max 20 years)"}
        )
    db.commit()
    db.refresh(a)
    if scale_changed:
...
def duplicate(
    ...
    orig = db.query(Record).filter(Record.id == record_id, Record.user_id == user.id).first()
    if not orig:
        raise HTTPException(404, {"code": "RECORD_NOT_FOUND", "message": "Not found"})
    a = Record(
        user_id=user.id,
...
def _missing_fields(rows: list[RecordRow]) -> list[dict]:
    ...
    missing = []
    for idx, row in enumerate(sorted(rows, key=lambda f: f.position), 1):
        if not row.company_id:
            missing.append(
                {"row": idx, "id": row.id, "field": "company", "name": row.name_snapshot}
            )
    return missing
...
def export_file(
    ...
    if format == "excel":
        ...
        return StreamingResponse(
            ...
            headers={"Content-Disposition": f"attachment; filename={TXT['file']}-{record_id}.xlsx"},
        )
    else:
        ...
        return StreamingResponse(
            ...
            headers={"Content-Disposition": f"attachment; filename=revision-{record_id}.pdf"},
        )
