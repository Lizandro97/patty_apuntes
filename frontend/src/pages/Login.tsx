...
const schema = z.object({ email: z.string().email("Email no válido"), password: z.string().min(6, "Mínimo 6 caracteres") })
...
export function Login() {
  ...
  const onSubmit = async (v: any) => {
    try {
      ...
      nav("/")
    } catch (e:any) { setErr(e.response?.data?.detail ?? "Error al iniciar sesión") }
  }
      ...
      <div className="flex-1 relative overflow-hidden flex flex-col justify-between p-8 lg:p-12 xl:p-16 min-h-[50dvh] lg:min-h-[100dvh] bg-gradient-to-br from-violet-600 via-indigo-600 to-violet-700 text-white">
        ...
        <div className="relative">
          <div className="inline-flex items-center gap-2 rounded-full bg-white/15 border border-white/20 px-3 py-1.5 backdrop-blur">
            ...
            <span className="text-[11px] tracking-[0.14em] uppercase font-semibold text-white">Patty Apuntes — 2026</span>
          </div>
        </div>
        ...
        <div className="relative max-w-[560px] py-8">
          <h1 className="font-display text-[44px] lg:text-[58px] leading-[0.9] tracking-tight">
            Tus números, <br />
            <span className="italic font-normal text-violet-200">en orden.</span>
          </h1>
          <p className="mt-5 text-[15px] leading-relaxed text-violet-100 max-w-[44ch]">
            Un papel tranquilo para tus revisiones. Marca, colorea y ordena sin ruido — como escribir a mano, con la precisión de una hoja infinita.
          </p>
          ...
          <div className="mt-10 hidden lg:block">
            <div className="bg-white/10 border border-white/15 backdrop-blur rounded-[1.5rem] p-3">
              <div className="bg-white rounded-[1.1rem] p-4 shadow-soft-lg">
                <div className="flex items-center gap-2 text-xs font-medium text-[#64748B]"><div className="w-2 h-2 rounded-full bg-emerald-500"/> Vista previa — Editor vivo</div>
                ...
                <div className="mt-3 flex items-center gap-2 text-[11px] text-[#64748B]"><Check size={12} strokeWidth={1.5}/> Marca con clic · clic derecho colorea</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="relative flex items-center gap-3 text-xs text-violet-200">
          <span className="w-8 h-px bg-white/30"/> Papel, orden y calma — hecho para Patty
        </div>
      </div>
          ...
          <div className="bezel-outer">
            <div className="bezel-inner p-8 lg:p-9">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] tracking-[0.14em] uppercase font-semibold text-[#94A3B8]">Acceso</span>
              </div>
              
              <h2 className="font-display text-[28px] leading-none tracking-tight text-[#1E293B] mt-6">Bienvenida de nuevo</h2>
              <p className="text-sm text-[#64748B] mt-2">Entra para continuar tus revisiones.</p>
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="mt-7 space-y-4">
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between"><label className="text-xs font-medium tracking-wide text-[#1E293B]">Contraseña</label><span className="text-xs text-[#94A3B8] flex items-center gap-1"><ShieldCheck size={12} strokeWidth={1.5}/> Segura</span></div>
              ...
              <p className="text-sm text-center mt-6 text-[#475569]">¿No tienes cuenta? <Link to="/register" className="font-semibold text-[#6366F1] hover:text-[#5458E8] underline underline-offset-4">Regístrate</Link></p>
            </div>
          </div>
          <p className="text-center text-xs text-[#94A3B8] mt-4">© {new Date().getFullYear()} Patty Apuntes — Hecho con calma</p>
        </div>
      </div>
    </div>
  )
