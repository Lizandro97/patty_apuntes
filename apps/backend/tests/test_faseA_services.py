"""Fase A: capa services + contratos alineados TS/Python + scoping devices."""
...
def test_calc_progress_half_up_como_math_round():
...
def test_sanitize_paridad_ts_solo_strings():
...
def test_remap_descarta_stale_y_conserva_header():
...
def test_missing_fields_requiere_empresa():
    rows = [
...
def test_export_strings_y_limite():
...
def test_resolve_strategy_invalida_422():
...
def test_devices_scoped_por_usuario():
    ...
    # B no puede revocar el dispositivo de A.
