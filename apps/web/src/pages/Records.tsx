...
function CreateBar({ title, setTitle, pending, onCreate }: {
  title: string; setTitle: (v: string) => void; pending: boolean; onCreate: () => void
}) {
  const { t } = useTranslation()
  return (
    <div className="flex gap-2 max-md:flex-col bg-[var(--surface)] border border-[var(--border)] rounded-xl p-2">
      <Input aria-label={t("records.newTitlePlaceholder")} placeholder={t("records.newTitlePlaceholder")} value={title} onChange={e=>setTitle(e.target.value)} onKeyDown={e=>{ if(e.key==="Enter") { if (e.nativeEvent.isComposing) return; if(!pending) onCreate() } }} className="w-64 max-md:w-full bg-[var(--bg)] border-[var(--border)] text-[var(--text)] placeholder:text-[var(--text-dim)] rounded-lg" />
      <Button onClick={onCreate} disabled={pending} className="bg-[var(--accent)] hover:brightness-110 text-[var(--on-accent)] rounded-lg min-h-[44px]"><Plus size={16}/> {t("records.create")}</Button>
    </div>
  )
}

function RecordsSearch({ q, setQ, sort, sortLabel, sortOpen, onToggleSort, sortBtnRef }: {
  q: string; setQ: (v: string) => void; sort: string; sortLabel: (v: string) => string
  sortOpen: boolean; onToggleSort: () => void; sortBtnRef: React.RefObject<HTMLButtonElement | null>
}) {
  const { t } = useTranslation()
  return (
    <div>
...
function RecordsTable({ list, isPending, isError, refetch, page, pages, total, rangeStart, rangeEnd, onPage, emptyLabel, onOpen, onRename, onDownload, onDelete }: {
  list: any[]; isPending: boolean; isError: boolean; refetch: () => void
  page: number; pages: number; total: number; rangeStart: number; rangeEnd: number; onPage: (p: number) => void; emptyLabel: string
  onOpen: (a: any) => void; onRename: (a: any) => void; onDownload: (a: any) => void; onDelete: (a: any) => void
}) {
            ...
            <tr key={a.id} className="hover:bg-[var(--surface-2)] transition">
              ...
              <td className="p-3 text-center whitespace-nowrap">
                <span className="inline-flex gap-1.5 justify-center items-center">
                  <Button variant="outline" onClick={()=>onRename(a)} title={t("records.rename")} aria-label={t("records.renameFileAria")} aria-haspopup="dialog" className="min-w-[44px] min-h-[44px] p-0 shrink-0 bg-[var(--bg)] border-[var(--border)] text-[var(--text-dim)] hover:text-[var(--text)]"><Pencil size={16}/></Button>
                  <Button variant="ghost" title={t("records.download")} aria-label={t("records.downloadFileAria", { title: a.title })} aria-haspopup="dialog" onClick={()=>onDownload(a)} className="min-w-[44px] min-h-[44px] p-0 shrink-0 text-[var(--text-dim)] hover:text-[var(--text)]"><Download size={16}/></Button>
                  <Button variant="ghost" title={t("records.delete")} aria-label={t("records.deleteFileAria", { title: a.title })} aria-haspopup="dialog" onClick={()=>onDelete(a)} className="min-w-[44px] min-h-[44px] p-0 shrink-0 text-[var(--text-dim)] hover:text-[var(--danger)]"><Trash2 size={16}/></Button>
                </span>
              </td>
            </tr>
          ))}
      ...
      {pages > 1 && (
        <div className="flex items-center justify-between px-4 py-2 border-t border-[var(--border)] text-xs text-[var(--text-dim)]">
          ...
          <span className="flex gap-1">
            <Button variant="outline" disabled={page === 0} onClick={() => onPage(page - 1)} className="min-h-[44px] px-4 rounded-lg">←</Button>
            <Button variant="outline" disabled={page >= pages - 1} onClick={() => onPage(page + 1)} className="min-h-[44px] px-4 rounded-lg">→</Button>
          </span>
        </div>
      )}
      {list.length===0 && <div className="text-center py-12 text-sm text-[var(--text-dim)] border-t border-[var(--border)]">{emptyLabel}</div>}
    </div>
  )
}

function SortMenu({ pos, sort, sortLabel, onPick }: {
  pos: { top: number; left: number; width: number }; sort: string; sortLabel: (v: string) => string; onPick: (v: "date" | "progress" | "title") => void
}) {
  const { t } = useTranslation()
  return createPortal(
    <div data-sortmenu role="menu" aria-label={t("records.sortLabel")} style={{ position: "fixed", top: pos.top, left: pos.left, width: pos.width, zIndex: 70 }} className="bg-[var(--surface)] border border-[var(--border)] rounded-xl shadow-[0_16px_40px_rgba(0,0,0,0.35)] p-1.5">
...
function DownloadDialog({ rec, onPick, onClose }: {
  rec: RecordItem | null; onPick: (rec: RecordItem, fmt: "pdf" | "excel") => void; onClose: () => void
}) {
  const { t } = useTranslation()
  const ref = useRef<HTMLDialogElement>(null)
  useDialogSync(ref, rec !== null, onClose)
  return (
    <dialog ref={ref} onClose={onClose} aria-label={t("records.downloadTitle", { title: rec?.title ?? "" })} className="bg-transparent p-4 backdrop:bg-black/60">
      <div className="w-[360px] max-w-[calc(100vw-3rem)] bg-[var(--surface)] border border-[var(--border)] rounded-2xl shadow-2xl p-5">
        <div className="flex items-start justify-between gap-2">
          <div className="text-sm font-semibold text-[var(--text)]">{t("records.downloadTitle", { title: rec?.title ?? "" })}</div>
        ...
        <div className="mt-4 space-y-2">
          <Button onClick={()=>{ if (rec) onPick(rec,"pdf"); onClose() }} className="w-full min-h-[44px] bg-[var(--accent)] hover:brightness-110 text-[var(--on-accent)] rounded-xl justify-start gap-2.5 px-4"><Download size={15}/>{t("records.downloadPdf")}</Button>
          <Button variant="outline" onClick={()=>{ if (rec) onPick(rec,"excel"); onClose() }} className="w-full min-h-[44px] rounded-xl justify-start gap-2.5 px-4"><Download size={15}/>{t("records.downloadExcel")}</Button>
        </div>
      </div>
    </dialog>
  )
...
function DeleteRecordDialog({ rec, pending, onConfirm, onClose }: {
  rec: RecordItem | null; pending: boolean; onConfirm: () => void; onClose: () => void
}) {
  const { t } = useTranslation()
  const ref = useRef<HTMLDialogElement>(null)
  useDialogSync(ref, rec !== null, onClose)
  return (
    <dialog ref={ref} onClose={onClose} aria-label={t("records.deleteFileAria", { title: rec?.title ?? "" })} className="bg-transparent p-4 backdrop:bg-black/60">
      <div className="w-[360px] max-w-[calc(100vw-3rem)] bg-[var(--surface)] border border-[var(--border)] rounded-2xl shadow-2xl p-5">
        <div className="text-sm font-semibold text-[var(--text)]">{t("records.deleteTitle", { title: rec?.title ?? "" })}</div>
        <p className="text-xs text-[var(--text-dim)] mt-1 mb-4">{t("records.deleteDesc")}</p>
        <div className="flex gap-2">
          <Button variant="outline" onClick={onClose} className="flex-1 min-h-[44px] rounded-xl">{t("records.cancel")}</Button>
          <Button onClick={onConfirm} disabled={pending} className="flex-1 min-h-[44px] bg-[var(--danger)] hover:brightness-110 text-[var(--on-accent)] rounded-xl">{t("records.confirmDelete")}</Button>
        </div>
      </div>
    </dialog>
  )
...
function RenameDialog({ rec, title, setTitle, pending, onSave, onClose }: {
  rec: RecordItem | null; title: string; setTitle: (v: string) => void; pending: boolean; onSave: () => void; onClose: () => void
}) {
  const { t } = useTranslation()
  const ref = useRef<HTMLDialogElement>(null)
  useDialogSync(ref, rec !== null, onClose)
  const save = () => { if (title.trim() && !pending) onSave() }
  return (
    <dialog ref={ref} onClose={onClose} aria-label={t("records.renameTitle", { title: rec?.title ?? "" })} className="bg-transparent p-4 backdrop:bg-black/60">
      <div className="w-[360px] max-w-[calc(100vw-3rem)] bg-[var(--surface)] border border-[var(--border)] rounded-2xl shadow-2xl p-5">
        <div className="flex items-start justify-between gap-2">
          <div className="text-sm font-semibold text-[var(--text)]">{t("records.renameTitle", { title: rec?.title ?? "" })}</div>
          <button onClick={onClose} aria-label={t("common.close")} className="w-9 h-9 -mt-1 -mr-1 grid place-items-center rounded-lg text-[var(--text-dim)] hover:text-[var(--text)] hover:bg-[var(--surface-2)]"><X size={16}/></button>
        </div>
        <div className="mt-4 space-y-3">
          <Input aria-label={t("records.renameFileAria")} value={title} onChange={e=>setTitle(e.target.value)} onKeyDown={e=>{ if(e.key==="Enter") { if (e.nativeEvent.isComposing) return; save() } }} className="h-11 bg-[var(--bg)] border-[var(--border)] text-[var(--text)]" />
          <div className="flex gap-2">
            <Button variant="outline" onClick={onClose} className="flex-1 min-h-[44px] rounded-xl">{t("records.cancel")}</Button>
            <Button onClick={save} disabled={!title.trim() || pending} className="flex-1 min-h-[44px] bg-[var(--accent)] hover:brightness-110 text-[var(--on-accent)] rounded-xl">{t("records.save")}</Button>
          </div>
        </div>
      </div>
    </dialog>
...
export function Records() {
  ...
  const rangeStart = safePage * PAGE + 1
  ...
  const openRename = (a: RecordItem) => { setRenameRec(a); setRenameTitle(a.title) }
  return (
    <div className="flex-1 bg-[var(--bg)] p-6 overflow-auto">
      <div className="max-w-[1000px] mx-auto space-y-4">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          ...
          <CreateBar title={title} setTitle={setTitle} pending={create.isPending} onCreate={()=>create.mutate()} />
        </div>
        <RecordsSearch q={q} setQ={setQ} sort={sort} sortLabel={sortLabel} sortOpen={sortOpen} onToggleSort={()=>sortOpen ? closeSort(true) : openSort()} sortBtnRef={sortBtnRef} />
        <RecordsTable
          list={list} isPending={isPending} isError={isError} refetch={()=>refetch()}
          page={safePage} pages={pages} total={filtered.length} rangeStart={rangeStart} rangeEnd={rangeEnd} onPage={setPage}
          emptyLabel={q ? t("records.noResults") : t("records.noFiles")}
          onOpen={(a)=>nav(`/editor/${a.id}`)} onRename={openRename} onDownload={setDlRec} onDelete={setDelRec}
        />
        {filtered.length > PAGE && (
          <div className="px-1 text-xs text-[var(--text-dim)]">{rangeStart}–{rangeEnd} / {filtered.length}</div>
        )}
      </div>
      <DownloadDialog rec={dlRec} onPick={download} onClose={()=>setDlRec(null)} />
      <DeleteRecordDialog rec={delRec} pending={del.isPending} onConfirm={()=>{ if (delRec) del.mutate(delRec.id); setDelRec(null) }} onClose={()=>setDelRec(null)} />
      <RenameDialog rec={renameRec} title={renameTitle} setTitle={setRenameTitle} pending={rename.isPending} onSave={()=>renameRec && rename.mutate({id: renameRec.id, title: renameTitle.trim()})} onClose={()=>setRenameRec(null)} />
      {sortOpen && sortPos && <SortMenu pos={sortPos} sort={sort} sortLabel={sortLabel} onPick={(v)=>{ setSort(v); closeSort(true) }} />}
    </div>
  )
}
