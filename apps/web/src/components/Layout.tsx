...
function MobileBar() {
  ...
  const setMobileOpen = useUiStore((s) => s.setMobileOpen)
  return (
    <header className="lg:hidden min-h-[52px] flex items-center gap-2 px-3 pt-[env(safe-area-inset-top)] bg-[var(--bg)] border-b border-[var(--border)] shrink-0 no-print">
      <button
        onClick={() => setMobileOpen(true)}
        title={t("nav.openMenu")}
        aria-label={t("nav.openMenu")}
        className="w-11 h-11 flex items-center justify-center rounded-lg text-[var(--text-dim)] hover:bg-[var(--surface)] hover:text-[var(--text)] transition"
      >
        <Menu size={18} />
      </button>
